...
public:
    ...
    void remove(string name) // doesnt properly work. Gets rid of everything in the stack but not the stack itself.
    {
        ...
        else
        {
            ...
            else // Case: everything else
            {
                ...
                toRemove = nullptr;             // sets pointer of node to null.
                if (length == 1)
                    first = current; // likewise for first.
            }
            length--;
            if (current->link == nullptr)
                last = current; // makes sure if the current is last, to set last to it.
        }
    }
    void remove(int pos) // doesnt properly work. Gets rid of everything in the stack but not the stack itself.
    {
        Node<Type> *toRemove = search(pos);
        // cout << "\tTo Remove: " << toRemove->data->getName() << endl; // debug
        if (toRemove == nullptr) // Case: not found
            cout << "Stack at position " << pos << " not found." << endl;
        else
        {
            Node<Type> *current = first;
            if (current == toRemove) // Case: removing head node
    ...
    Node<Type> *search(string name) // returns the node of whatever is being searched if found
    {
        Node<Type> *current = first;
    ...
    friend ostream &operator<<(ostream &os, const LinkedList &ll)
    {
        ...
        while (current != nullptr)
        {
            os << i << ". " << current->data->getName() << endl;
            current = current->link;
            i++;
        }
        current = nullptr;
